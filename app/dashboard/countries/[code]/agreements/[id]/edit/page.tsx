...
export const generateMetadata = ({
  ...
  return {
    title: agreement
      ? `Edit ${agreement.name} | ${country?.name} Agreements`
      : "Edit Agreement | Staff Portal",
    description: agreement
      ? `Edit details of ${agreement.name}`
      ...
      <div className="space-y-6">
        ...
        <div className="flex items-center text-sm text-muted-foreground">
          <Button variant="ghost" size="icon" className="h-8 w-8 mr-2" asChild>
            <Link
              href={`/dashboard/countries/${params.code}/agreements/${params.id}`}
            >
            ...
            <DropdownMenuContent align="start" className="w-56">
              {mockCountries.map((c) => (
                <DropdownMenuItem key={c.code} asChild>
                  <Link
                    href={`/dashboard/countries/${c.code}/agreements/${params.id}/edit`}
                    className="flex items-center"
                  >
                    <img
        ...
        <div className="flex items-center space-x-3">
          ...
          <div>
            ...
            <p className="mt-1 text-gray-600">
              Edit details for {agreement.name}
            </p>
          </div>
        </div>
        ...
        <Card>
          ...
          <CardContent>
            <EditAgreementForm agreement={agreement} country={country} />
          </CardContent>
        </Card>
      </div>
    </DashboardLayout>
